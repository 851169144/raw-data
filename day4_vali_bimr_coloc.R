...
### 反向孟德尔
# 暴露
bimr_SLE<- extract_instruments('ukb-d-I9_CORATHER', p1=5e-08, clump=TRUE)

#提取结局数据
outcome_gene<- extract_outcome_data(snps=bimr_SLE$SNP, outcomes="eqtl-a-ENSG00000169896")
...
## 下载
data <- vcfR::read.vcfR("./eqtl-a-ENSG00000169896.vcf/eqtl-a-ENSG00000169896.vcf")
...
gt$`eqtl-a-ENSG00000169896`[1]

##!!!
gt=separate(gt,col='eqtl-a-ENSG00000169896',into = c('ES', 'SE',
                                       'LP','AF','SS',
                                       'ID'),sep = '\\:')
...
## 上下1mkb
eqtl=eqtl[eqtl$pos > 28724374-1000000 ,]
eqtl=eqtl[eqtl$pos < 28724374+1000000 ,]
...
coloc.abf(dataset1=list(pvalues=input$pvalues_gwas, type="cc", s=0.04, N=361,194),
                    dataset2=list(pvalues=input$pvalues_eqtl, type="quant", N=31,684),
                    MAF=input$MAF)
